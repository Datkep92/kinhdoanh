...
async function initHKDPage() {
    try {
        ...
        displayHKDInfo();
        displayProducts(); // Render sản phẩm trước
        initCart();
        initSidebar();
        initHKDRealtimeSync();
        ...
        // Tải ảnh SAU KHI đã render sản phẩm
        setTimeout(async () => {
            await loadAllProductImages();
        }, 200); // Delay để DOM render xong
        // Thêm initEditMode() sau khi init xong
        setTimeout(() => {
...
function displayProducts(category = 'All') {
        ...
        return `
            <div class="product-card" data-product-id="${product.id}">
                ...
                <div class="product-controls">
                    <button 
                        class="btn-decrease ${cartQuantity > 1 ? 'active' : ''}"
    ...
    // Load ảnh async cho từng sản phẩm
    setTimeout(() => {
 ...
 */
async function loadAndDisplayProductImage(productId) {
    try {
        // Kiểm tra xem sản phẩm có tồn tại trong DOM không
        const productCard = document.querySelector(`.product-card[data-product-id="${productId}"]`);
        if (!productCard) {
            console.warn(`⚠️ Không tìm thấy thẻ sản phẩm ${productId} trong DOM`);
            return;
        }
        ...
        if (!imageData || !imageData.imageData) {
            // Không có ảnh, chỉ cần đảm bảo có loader
            ensureImageLoader(imageContainer);
        ...
        img.onload = () => {
            ...
            // Thêm class has-image cho container
            imageContainer.classList.add('has-image');
            productCard.classList.add('has-image');
            
            // Ẩn loader (CSS sẽ tự động ẩn khi có .has-image)
            console.log(`✅ Ảnh ${productId} đã load xong`);
        };
        
        img.onerror = () => {
            console.error(`❌ Lỗi tải ảnh sản phẩm ${productId}`);
            imageContainer.classList.remove('has-image');
            productCard.classList.remove('has-image');
            
            // Hiển thị icon lỗi trong loader
            const loader = imageContainer.querySelector('.image-loader');
            if (loader) {
                loader.innerHTML = '<i class="fas fa-exclamation-triangle"></i>';
                loader.style.color = '#fbbf24';
            }
        };
        
        // Sử dụng thumbnail nếu có, ngược lại dùng imageData chính
        img.src = imageData.thumbnail || imageData.imageData;
        
        // Xóa các img element cũ trong container
        const oldImages = imageContainer.querySelectorAll('img');
        ...
        // Nếu ảnh đã được cache (có thể load ngay), trigger onload
        if (img.complete) {
            img.onload();
        }
        
    } catch (error) {
        console.error(`❌ Lỗi tải ảnh sản phẩm ${productId}:`, error);
...
/**
 * Đảm bảo container ảnh có loader
 * @param {HTMLElement} imageContainer - Container ảnh
 */
function ensureImageLoader(imageContainer) {
    // Kiểm tra đã có loader chưa
    if (!imageContainer.querySelector('.image-loader')) {
        const loader = document.createElement('div');
        loader.className = 'image-loader';
        loader.innerHTML = '<i class="fas fa-image"></i>';
...
/**
 * Tải ảnh cho tất cả sản phẩm (optimized)
 */
async function loadAllProductImages() {
    try {
        ...
        // Lấy tất cả ảnh của HKD một lần
        const allImages = await getProductImagesByHKD(currentHKD.id);
        const imageMap = {};
        
        // Tạo map để truy cập nhanh
        allImages.forEach(img => {
            if (img && img.productId) {
                imageMap[img.productId] = img;
            }
        });
        
        // Lấy tất cả thẻ sản phẩm trong DOM
        const productCards = document.querySelectorAll('.product-card');
        
        // Xử lý từng thẻ sản phẩm
        productCards.forEach(card => {
            const productId = card.getAttribute('data-product-id');
            if (!productId) return;
            
            const imageData = imageMap[productId];
            if (!imageData) {
            ...
            // Xóa loader cũ
            const oldLoader = imageContainer.querySelector('.image-loader');
            if (oldLoader) {
                imageContainer.removeChild(oldLoader);
            }
            ...
            // Trigger khi ảnh load xong
            img.onload = () => {
                imageContainer.classList.add('loaded');
            };
        });
        
        console.log(`✅ Đã tải ${allImages.length} ảnh sản phẩm`);
...
function showAllProducts() {
    console.log('📦 Hiển thị All sản phẩm');
...
async function deleteHKDCategory(categoryId) {
    ...
    const confirmed = await Utils.confirm(
        `Xóa danh mục "${category.name}"? All sản phẩm trong danh mục sẽ bị xóa.`
    );
    if (!confirmed) return;
...
function setupHKDEventListeners() {
    ...
    // Product click - SỬ DỤNG EVENT DELEGATION
     // Product click - CHỈ CLICK VÀO VÙNG KHÔNG PHẢI NÚT
    const productGrid = document.getElementById('productGrid');
    if (productGrid) {
        productGrid.addEventListener('click', (e) => {
            // Kiểm tra nếu click vào nút tăng/giảm thì bỏ qua
            if (e.target.closest('.btn-increase') || 
                e.target.closest('.btn-decrease') ||
                e.target.closest('.product-controls')) {
                return;
            }
            
            // Nếu click vào sản phẩm (thẻ cha)
...
// Thêm vào hàm setupProductListenersForCategory hoặc tạo listener mới
async function setupImageChangeListeners() {
    // ... (sẽ thêm realtime listener cho ảnh khi cần)
}

// Thêm vào sự kiện khi ảnh được cập nhật
window.addEventListener('productImageUpdated', (event) => {
    const { productId } = event.detail;
    if (productId) {
        reloadProductImage(productId);
    }
});

// Helper để trigger event
function triggerImageUpdate(productId) {
...
/**
 * Reload ảnh cho sản phẩm cụ thể
 * @param {string} productId - ID sản phẩm
 */
 ...
 * Reload tất cả ảnh
 */
async function reloadAllProductImages() {
